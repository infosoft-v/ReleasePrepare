...
Процедура ЛогСообщения(ТекстСообщения, УровеньСообщения, ВыводНаКонсоль)
	Перем ИНФОРМАЦИЯ, СООБЩЕНИЕ, ОШИБКА;
	ИНФОРМАЦИЯ = 1;
	СООБЩЕНИЕ = 2;
	ОШИБКА = 3;
	Если УровеньСообщения = 0 Тогда
		Лог.Отладка(ТекстСообщения);
	ИначеЕсли УровеньСообщения = ИНФОРМАЦИЯ Тогда
		Лог.Информация(ТекстСообщения);
	ИначеЕсли УровеньСообщения = СООБЩЕНИЕ Тогда
		Лог.Предупреждение(ТекстСообщения);
	ИначеЕсли УровеньСообщения = ОШИБКА Тогда
...
Функция ОбработкуРазобрать(ПутьКФайлу, КаталогДампа)
	...
	Если КодВозврата <> 0 Тогда
		ТекстСообщения = "проблемы при выгрузке файлов из обработки. КодВозврата = "+ Строка(КодВозврата);
		ЛогСообщения(ТекстСообщения, 3, Истина);
...
Функция ДанныеВерсии(ИмяФайла)
	...
	Если НЕ НастройкиПроекта.Свойство("ПоискНомераВерсии") Тогда
		ЛогСообщения(
			...
			|отсутствует секция ""ПоискНомераВерсии"" с параметрами поиска номера версии.
			|Для формирования имена мастер-обработки берем строку x-xx-xxx",
			1,
			Ложь);
		ВерсияНомер = "x-xx-xxx";
		Возврат ВерсияНомер;
	КонецЕсли;
	Если НЕ НастройкиПроекта.ПоискНомераВерсии.Свойство("ПаттернПоиска") Тогда
		ЛогСообщения(
			"В настройках секции ""ПоискНомераВерсии""
			|отсутствует паттерн поиска в коде номера версии.
			|Для формирования имена мастер-обработки берем строку x-xx-xxx",
			1,
			Ложь);
		ВерсияНомер = "x-xx-xxx";
		Возврат ВерсияНомер;
	КонецЕсли;
	Если НЕ НастройкиПроекта.ПоискНомераВерсии.Свойство("НомерГруппы") Тогда
		ЛогСообщения(
			"В настройках секции ""ПоискНомераВерсии""
			|отсутствует номер группы для получения номера версии из найденного фрагмента.
			|Для формирования имена мастер-обработки берем строку x-xx-xxx",
			1,
			Ложь);
		ВерсияНомер = "x-xx-xxx";
...
Функция ОбработкуСобрать(ПутьКФайлуСборкиПроекта, ПутьКФайлуМастерОбработки)
	...
	КодВозврата = Команда.Исполнить();
	Если КодВозврата <> 0 Тогда
		ТекстСообщения = "проблемы при загрузке файлов в обработки. КодВозврата = " + Строка(КодВозврата);
		ЛогСообщения(ТекстСообщения, 3, Истина);
...
ТекстСообщения = СтрШаблон("%1 - запуск с -prj=%2 -cfg=%3", Строка(ТекущаяДата()), ПроектКлюч, ФайлНастроек);
Лог.Информация(ТекстСообщения);
...
Если НЕ НастройкиЗапуска.Свойство("Programs") Тогда
	...
	Exit(0);
КонецЕсли;
Если НЕ НастройкиЗапуска.Programs.Свойство("v83") Тогда
